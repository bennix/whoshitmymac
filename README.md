...
[下载已签名并经过 Apple 公证的 v1.1.1 DMG](https://github.com/bennix/whoshitmymac/releases/download/v1.1.1/WhoShitOnMyMac-1.1.1.dmg)
...
- **微信括号副本**：只处理 `xwechat_files` 里文件名末尾带 `(1)` / `(2)` 的附件，并且 MD5 与某份无括号原件相同；标题中间的中文括号不算副本。勾选后可单独移到废纸篓，不影响其他待删除项。
